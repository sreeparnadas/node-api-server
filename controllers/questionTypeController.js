...
const QuestionType = models.QuestionType // Use models' ClassName here

let self = {}

self.getAll = async (req, res) => {
    try{
        let data = await QuestionType.findAll({})    
...
self.create = async (req, res) => {
    ...
    try{
        ...
        let data = await QuestionType.create(newdata)
...
self.getById = async (req,res) => {
    try{
        ...
        let data = await QuestionType.findByPk(id, {
        attributes: {
...
self.update = async (req,res) => {
    try{
        id = req.params.id
        body = req.body
        let data = await QuestionType.update(body, {
            where: {id: id}
        })
        if(data[0]==0){
...
self.delete = async (req,res) => {
    try{
        let id = req.params.id
        let data = await QuestionType.destroy({
            where: {
                id: id
            }
        })
